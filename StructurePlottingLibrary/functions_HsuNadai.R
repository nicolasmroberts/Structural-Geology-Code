...
#' @param maxEs the max octahedral shear strain to plot. a positive number.
blankHsuNadaiPlot = function(plotArcs = TRUE, numArcs = 4, plotLines = TRUE, maxEs = 3) {
     ...
     subLines <- data.frame(x1 = c(0,0,0,0,0,0), 
                            x2 = c(maxEs*cos(9*pi/24),maxEs*cos(10*pi/24), maxEs*cos(11*pi/24), maxEs*cos(13*pi/24), maxEs*cos(14*pi/24), maxEs*cos(15*pi/24)),
                            y1 = c(0,0,0,0,0,0), 
                            y2 = c(maxEs*sin(9*pi/24),maxEs*sin(10*pi/24), maxEs*sin(11*pi/24), maxEs*sin(13*pi/24), maxEs*sin(14*pi/24), maxEs*sin(15*pi/24)),
...
plotHsuNadai = function(plot = NULL, logAs, maxOct = NULL, alpha = c(1), color = c("black"), shape = c(20), size = 1, annotateWith = NULL, hidePoints = FALSE, colorBy = NULL) {
     
     if (length(logAs) > 1) {
          if (length(shape) != length(logAs)) {
               shape = rep(shape[1],length(logAs))
          }
          
          if (length(alpha) != length(logAs)) {
               alpha = rep(alpha[1],length(logAs))
          }
          
          if (length(color) != length(logAs)) {
               color = rep(color[1],length(logAs))
          }
          if (length(size) != length(logAs)) {
               size = rep(size[1],length(logAs))
          }
     }
     ...
     if (is.null(plot) == TRUE){
          if (is.null(maxOct) == TRUE){
               plot = blankHsuNadaiPlot(maxEs = ceiling(max(octs)))
          } else {
               plot = blankHsuNadaiPlot(maxEs = maxOct)
          }
     ...
     if(hidePoints == FALSE) {
          if(is.null(colorBy) == TRUE){
               for (i in 1:length(logAs)) {
                    ...
                    #plot = plot + geom_point(aes(x=lodes, y=oct), data = xy, size = 1, alpha = 1, color = 1, shape = 22)
